...
namespace MyUtil
{
	...
	HRESULT CreateDevice()
	{
		...
		// IDirect3D9 객체 얻기가 실패하면 NULL 이 참조됨
		if (d3d9 == NULL)
		{
		...
		d3d9->GetDeviceCaps(
			...
			D3DDEVTYPE_HAL,			// # 디바이스 타입 설정 그냥 D3DDEVTYPE_HAL #
			&caps					// # 디바이스 정보를 받아 올 D3DCAPS9 포인터 #
		);

		// # 그래픽 카드에서 지원되는 이방성 필터링 최대 단계 #
		MaxAnisotropic = caps.MaxAnisotropy;

		// # 정점 계산 처리방식을 지정할 플래그 값 #
		...
		HRESULT result = d3d9->CreateDevice(
			...
			vertexProcessing | D3DCREATE_MULTITHREADED,			//정점 처리 방식에 대한 플레그  ( D3DCREATE_MULTITHREADED 멀티 쓰레드에 대응되는 Device ) #
	...
	{
		POINT mousePos;
		GetCursorPos(&mousePos);	// # 마우스 Screen 상의 위치를 얻음 #

		// # Screen 상의 위치를 Client 영역 범위로 바꾼다. #
	...
	void SetMousePos(int x, int y)
	{
		...
		// # Client 상의 위치를 Screen 영역 범위로 바꾼다. #
	...
	// # 정점의 노말을 계산해주는 함수 #
	void ComputeNormal(
		D3DXVECTOR3* pNormals,			// # 결과로 받을 노말백터 배열 포인터 ( 노말벡터의 배열의 수는 정점 배열의 수와 같다 ) #
	...
	{
		// # 노말 결과 털고 시작 #
		ZeroMemory(pNormals, sizeof(D3DXVECTOR3) * verticesNum);
		...
		// # 최종적으로 NormalVector 정규화한다. #
		for (DWORD i = 0; i < verticesNum; i++)
		{
			D3DXVec3Normalize(&pNormals[i], &pNormals[i]);
		}
	}
	...
	{
		// # 탄젠트와 바이노말 털고 시작 #
		ZeroMemory(outTangents, sizeof(D3DXVECTOR3) * NumVertices);
	...
	// # 플룻의 비트 값을 손실하지 않은 채 DWORD 형으로 변환 #
	...
	// # 디바이스의 Tangent Color 값을 Texture에 쓴다. #
	void GetDeviceGrabTexture(LPDIRECT3DDEVICE9 pDevice, LPDIRECT3DTEXTURE9 grabTex)
	{
		...
		// # 백버퍼의 TargentSurface 의 내용을 m_pGrabTexture Surface 로 복사 #
		pDevice->StretchRect(
