...
void cParticle::Update(float timeDelta)
{
	...
	// 정규화 된 시간 값을 계산
	m_fNormalizeLiveTime = m_fDeltaLiveTime / m_fTotalLiveTime;
	if (m_fNormalizeLiveTime > 1.0f)
		m_fNormalizeLiveTime = 1.0f;
		...
		const VEC_SCALE& scales)
{
	...
	else if (m_fNormalizeLiveTime == 0.0f)
	{
		pOut->color = colors[0];
	...
	// 현재 라이브 타임 량에 따른 위치
	float position = m_fNormalizeLiveTime / interval;
	// position 의 값에 정수부분이 사용되는 배열의 인덱스가 되는 것이고,
	// 소수점 부분이 다음 배열값까지의 보간량이 된다.
	...
	// 현재 라이브 타임 량에 따른 위치
	position = m_fNormalizeLiveTime / interval;
