...
void cXMesh_Static::SetCamera(const cCamera* Camera)
{
	...
	sStaticMeshEffect->SetFloat("camFar", Camera->camFar);
	sStaticMeshEffect->SetFloat("camNear", Camera->camNear);
}
...
void cXMesh_Static::SetLighting(cLight** pLights, int lightNum)
{
	...
	//Effect 에 라이팅 셋팅
	sStaticMeshEffect->SetMatrixArray("matLights", matLight, 10);
	sStaticMeshEffect->SetInt("LightNum", lightNum);
}
...
void cXMesh_Static::SetLighting(std::vector<cLight*>* pLights)
{
	...
	//Effect 에 라이팅 셋팅
	sStaticMeshEffect->SetMatrixArray("matLights", matLight, 10);
	sStaticMeshEffect->SetInt("LightNum", pLights->size());
}
