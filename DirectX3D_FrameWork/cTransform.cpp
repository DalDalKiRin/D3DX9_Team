...
void cTransform::SetRotateWorld(float eAngleX, float eAngleY, float aAngleZ)
{
	...
	// # 사원 수에 의한 행렬 준비 #
	D3DXMATRIXA16 matRotate;
	D3DXMatrixRotationQuaternion(&matRotate, &quatRot);		// # 사원 수에 의한 회전 값으로 회전 행렬이 만들어진다. #
...
void cTransform::SetWorldMatrix(const D3DXMATRIXA16& matWorld)
{
	// # 월드 행렬로 셋팅 #
...
void cTransform::RenderGizmo(bool applyScale) const
{
	D3DXVECTOR3 worldPos = this->GetWorldPosition();
	D3DXVECTOR3 axis[3];
