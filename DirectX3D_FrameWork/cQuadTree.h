...
private:
	...
	cTerrain::LPTERRAINVERTEX	m_pTerrainVertices;					// 자신이 들고 있는 터레인의 정점 정보

	DWORD		m_Corners[4];	// 자신의 쿼드 트리의 각 코너 정점 인덱스
	DWORD		m_Center;		// 자신의 쿼드 트리의 중앙 정점 인덱스 (이게 -1 이라면 자식 없음)
	cQuadTree*	m_pChilds[4];	// 자신의 자식 쿼드 트리 (자신이 마지막 노드라면 다 NULL)

	D3DXVECTOR3	m_CenterPos;	// 자신의 쿼드 트리 중심 위치
	float		m_Radius;		// 자신의 쿼드 트리 영역 반지름
