	...
	int tileNum)					// 타일링 갯수
{
	...
	// 높이 맵에 대한 이미지 정보를 불러 옴
	D3DSURFACE_DESC sd;
	m_pHeightMap->GetLevelDesc(
		0,		// 얻어 올 밉맵 체인 레벨 0
		&sd		// 얻어 올 정보의 D3DSURFACE_DESC 구조체
		);

	// 가로 세로 정점 수를 구함
	m_nVerNumX = sd.Width + 1;	// 가로 정점 갯수는 높이 맵에 가로 해상도 + 1 과 같음.
	m_nVerNumZ = sd.Height + 1;	// 가로 정점 갯수는 높이 맵에 세로 해상도 + 1 과 같음.
...
bool cTerrain::IsIntersectRay(D3DXVECTOR3* pOut, const LPRay pRay)
{
	...
	// 여기까지 온다면 제일 거리가 가까운 Hit 위치가 나온다
	*pOut = hitPos;
...
bool cTerrain::GetSlant(D3DXVECTOR3* pOut, float gravityPower, float x, float z)
{
	...
	// 우측 벡터에서 법선 벡터를 외적한 결과가 경사면의 경사 방향이 된다.
...
HRESULT cTerrain::CreateTerrain(int smooth, int tileNum)
{
	...
	// 텍스쳐의 픽셀 정보를 얻기 위해 Texture 를 lock 한다.
	D3DLOCKED_RECT lockRect;
	m_pHeightMap->LockRect(
		0,				// lock 을 할 밉맵 레벨 0
		&lockRect,		// lock 으로 얻어온 D3DLOCKED_RECT 정점 구조체
		0,				// lock 을 일부 영역만 하고 싶은 경우 사각형RECT 구조체의 포인터를 주는데 전체 할꺼면 NULL
		0);				// lock 플래그 일단 0

	// lockRect->Pitch		lock 을 한 영역 이미지의 가로 byte 크기 (얻어온 바이트 크기는 다음을 성립한다 pitch % 4 == 0) < 3byte 컬러시 pitch byte 구하는 공식 (가로 픽셀 수 * 3 + 3) & ~3 = pitch >
	// lockRect->pBits		이미지 데이터가 시작되는 포인터 주소
	...
	for (int z = 0; z < m_nVerNumZ; z++)
	{
		for (int x = 0; x < m_nVerNumX; x++)
		{
			...
			// 정점의 x, z 위치 계산
			pos.x = (x - (m_nCellNumX * 0.5f)) * m_fCellScale;
			pos.z = (-z + (m_nCellNumZ * 0.5f)) * m_fCellScale;

			// 가로 마지막 라인이라면 (이전 왼쪽의 정점 Y 위치와 맞춘다)
			if (x == m_nVerNumX - 1)
			{
			...
			{
				// 해당 픽셀의 컬러 값을 얻는다.
				DWORD* pStart = (DWORD*)lockRect.pBits;	// DWORD 형으로 형 변환된 lock 된 이미지의 시작 주소
				DWORD dwColor = *(pStart + (z * lockRect.Pitch / 4) + x);

				// 각 커러 값을 0 ~ 1 사이의 실수로 나눈다.
			...
			m_pTerrainVertices[idx].normal = D3DXVECTOR3(0, 0, 0);
	...
	// 노말이랑 Binormal 이랑 Tangent 계산
	D3DXVECTOR3* poses = new D3DXVECTOR3[m_nTotalVerNum];		// 정점 위치
	...
	// 계산된거 대입
	for (int i = 0; i < this->m_nTotalCellNum; i++)
	{
	...
	// 정점의 형태를 알려주는 배열
	D3DVERTEXELEMENT9 vertElement[7];	// 배열의 정점 정보 갯수 + 1
...
void cTerrain::SmoothTerrain(int passed)
{
	...
	while (passed > 0)
	{
		...
		for (int z = 0; z < m_nVerNumZ; z++)
		{
			for (int x = 0; x < m_nVerNumX; x++)
			{
				int adjacentSections = 0;	// 몇 개의 정점과 평균 값을 내는지?
				float totalSections = 0.0f;	// 주변의 정점 높이 총 합은 얼만지?
