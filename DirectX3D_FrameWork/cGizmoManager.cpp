...
// # 선을 그리는 함수 #
void cGizmoManager::Line(const D3DXVECTOR3& startPos, const D3DXVECTOR3& endPos, DWORD color) const
{
	// # 이전의 월드 행렬을 기억하자. #
...
void cGizmoManager::WorldGrid(float cellSize, int cellNum) const
{
	...
	// # z 변환 #
	for (int i = 0; i < 4; i++) {
		D3DXVec3TransformCoord(&arrowVertice[i], &arrowPos[i], &matTrans);
	}
	// # 그린다. #
	this->Line(arrowVertice[0], arrowVertice[1], 0xff0000ff);
	this->Line(arrowVertice[2], arrowVertice[3], 0xff0000ff);
	...
	// # x 변환 #
	for (int i = 0; i < 4; i++) {
		D3DXVec3TransformCoord(&arrowVertice[i], &arrowPos[i], &matFinal);
	}
	// # 그린다. #
	this->Line(arrowVertice[0], arrowVertice[1], 0xffff0000);
	this->Line(arrowVertice[2], arrowVertice[3], 0xffff0000);

	// # y축 #
	D3DXMatrixTranslation(&matTrans, yEnd.x, yEnd.y, yEnd.z);
	D3DXMatrixRotationX(&matRotation, -90 * ONE_RAD);
	matFinal = matRotation * matTrans;
	...
	// # 그린다. #
	this->Line(arrowVertice[0], arrowVertice[1], 0xff00ff00);
	this->Line(arrowVertice[2], arrowVertice[3], 0xff00ff00);
...
// # 원을 그리는 함수 #
void cGizmoManager::Circle(const D3DXVECTOR3& centerPos, float radius, const D3DXVECTOR3& axis, DWORD color) const
{
	// # 이전의 월드 행렬을 기억하자. #
	D3DXMATRIXA16 matPrevWorld;
	m_pDevice->GetTransform(D3DTS_WORLD, &matPrevWorld);
...
void cGizmoManager::AABBBox(const D3DXVECTOR3& minPos, const D3DXVECTOR3& maxPos, DWORD color) const
{
