...
cVampirGentleman::cVampirGentleman()
{
}
...
void cVampirGentleman::Update(float timeDelta)
{
	this->pChar->Update(timeDelta);
}

void cVampirGentleman::Render()
{
	this->pChar->Render();
...
void cVampirGentleman::beginSetting()
{
	// # 무게와 속도를 정한다 #
	fWeight = 3.0f;
	fMoveSpeed = 0.0f;
...
void cVampirGentleman::MESH_LOAD()
{
	// # 크기 #
	...
	cXMesh_Skinned* pSkinned =
		...
		("../Resources/Enemy/VampirGentleman/VampirGentleman.X", &matCorrection);
...
void cVampirGentleman::AddTarget(cBaseObject * targetObj)
{
}
